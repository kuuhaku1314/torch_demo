...
def train():
    ...
    def get_params(vocab_size, num_hiddens, device):
        ...
        # 这是一个有一个隐藏层的rnn，假设输入数据为32条，有28个词元，隐藏层有512个隐藏单元，输出层有28个输出
    ...
    def rnn(inputs, state, params):
        ...
        # (28X512), (512X512), (512), (512X28), (28)
        W_xh, W_hh, b_h, W_hq, b_q = params
        H, = state  # 形状(批量大小，隐藏单元数) (32, 512)
        outputs = []
        # X的形状：(批量大小，词表大小) (32X28)
        # 这里处理有点特别，每次是处理一个时间步的批量数据
        for X in inputs:
            # (32X28) X (28X512) + (32, 512) X (512X512)-> (32X512) 中间隐藏层和隐状态输出之和
            # 需要注意的是，使用顺序分区时，每个批次中对应位置的样本向量，下一个批次里对应位置的样本向量是连续的
            # 所以隐状态可以学到连续的序列，而不只是单单长度为35的句子
            # 例如：batch_size=32, text_size=10000，则每个样本包含的连续文本长度约为 10000/32 ≈ 312
            # 公式：连续序列长度 ≈ (text_size - num_steps) / batch_size
            H = torch.tanh(torch.mm(X, W_xh) + torch.mm(H, W_hh) + b_h)
            # Y的形状：(批量大小，词表大小) (32X512) X (512X28) -> (32X28)
            Y = torch.mm(H, W_hq) + b_q
            outputs.append(Y)
        # 总结上面计算，这里有35个真实时间步数据，假设范围是[1, 35]，接下来根据隐状态和真实的第1个时间步数据，推断出第2个时间步数据
        # 逐次进行，则会推断出[2, 36]时间步数据，最后在外层用交叉熵方法，用预估的时间步数据和真实的[2, 36]时间步数据计算损失
        # 返回输出，输出是估计值(时间步数量X批量大小，词表大小)及隐状态
        # (1120, 28), (32, 512)
