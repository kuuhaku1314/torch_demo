...
def predict_seq2seq(net, src_sentence, src_vocab, tgt_vocab, num_steps,
    ...
    for _ in range(num_steps):
        # 此处每次把dec_state输入解码器，context会跟随训练发生变化，应该是有问题的，理应固定不变作为翻译的参考
...
class DotProductAttention(nn.Module):
    """缩放点积注意力，key和query是相同维度下使用，用点积将query映射到key的权重"""
    ...
    def forward(self, queries, keys, values, valid_lens=None):
        ...
        # 为了保持方差，这里在计算score后除以了math.sqrt(d)
...
class MultiHeadAttention(nn.Module):
    """多头注意力，融合了来自于多个注意力汇聚的不同知识，这些知识的不同来源于相同的查询、键和值的不同的子空间表示"""
...
class PositionalEncoding(nn.Module):
    """位置编码，由sin(x)和cos(x)交错拼接在嵌入向量后面，因为自注意力并行计算舍弃了位置信息，
    通过设置不同特征维度使用不同正余弦周期，巧妙地使得每个点的位置坐标都各不相同，且每个点的不同特征所对应坐标也不相同"""
